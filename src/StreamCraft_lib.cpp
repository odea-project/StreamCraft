...
sc::MS_SPECTRA_TARGETS sc::MS_ANALYSIS::get_spectra_targets(const sc::MS_TARGETS& targets) {
  ...
  // make int vector of indexes for sorting the _out vector by mz_out, then rt_out, then drift_out and then id_out
  ...
  std::sort(idx_sort.begin(), idx_sort.end(), [&](int i, int j) {
    ...
    if (drift_out[i] != drift_out[j]) return drift_out[i] < drift_out[j];
